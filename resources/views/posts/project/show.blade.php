...
@section('content')
    <div class="container mx-auto p-6 md:p-12">
        ...
        <!-- Imagen del Proyecto y Título -->
        <div class="relative mb-10">
            @if(isset($project->content[0]['value']))
                @php
                    $firstImage = $project->content[0]['value'];  // Guardamos la primera imagen
                @endphp
                <img class="w-full h-[100vh] object-cover rounded-lg shadow-lg transform transition duration-300 ease-in-out hover:scale-105" src="{{ $firstImage }}" alt="{{ $project->title }}">
            @else
            ...
            <div class="absolute inset-x-0 bottom-5 text-center">
                <h2 class="text-white text-4xl font-extrabold bg-black bg-opacity-60 p-3 rounded shadow-lg">{{ $project->title }}</h2>
            </div>
        </div>

        <!-- Descripción del Proyecto -->
        <div class="bg-gray-50 p-6 rounded-lg shadow-md mb-10">
            <h3 class="text-2xl font-semibold text-gray-800 mb-4">Descripción del Proyecto</h3>
            @php
                $firstText = null;
