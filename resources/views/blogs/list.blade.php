...
<div class="container mx-auto">
    ...
    <h1 class="text-3xl font-bold mb-6 text-gray-800">Mis Blogs</h1>

    <div class="mb-10">
        <h2 class="text-2xl font-semibold mb-3 text-green-700">Publicados</h2>

        @if($publicados->isEmpty())
            <p class="text-gray-500">No hay blogs publicados.</p>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">

                @foreach($publicados as $b)

                    @php
                        $primerTexto = null;
                        $primerImagen = null;

                        if (is_array($b->contenido)) {
                            foreach ($b->contenido as $item) {
                    ...
                    <div class="bg-white shadow rounded-lg overflow-hidden">
                        ...
                        <div class="p-4">
                            ...
                            {{-- TÍTULO --}}
                            <h3 class="text-lg font-bold text-gray-800">{{ $b->nombre }}</h3>
                            ...
                            <div class="mt-4 flex gap-2">
                                ...
                                <form action="{{ route('blogs.destroy', $b->id) }}"
                                      method="POST"
                                      onsubmit="return confirm('¿Eliminar blog?')">
                                    @csrf
                                    @method('DELETE')
