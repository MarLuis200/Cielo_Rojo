    ...
    <div id="modalContainer" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-start z-50 p-4">
        <div class="bg-white rounded-lg shadow-lg w-5/6 max-h-screen overflow-y-auto">
            ...
            <div class="flex justify-between items-center p-4 border-b bg-gray-100">
                <h5 class="text-lg font-semibold text-gray-800">Crear Blog</h5>
            ...
            <div class="p-4 grid grid-cols-2 gap-4">
                ...
                <!-- Columna izquierda -->
                <div>
                    <input type="text" id="blogName" placeholder="Nombre del blog"
...
function updatePreview() {
    ...
    document.querySelectorAll('#fieldsContainer div').forEach(field => {
        ...
        if (type === 'subtitle') el.innerHTML = `<h2 class="text-lg font-semibold">${value}</h2>`;
        if (type === 'text') el.innerHTML = `<p>${value}</p>`;
        if (type === 'image' && value)
            el.innerHTML = `<img src="${value}" class="w-full max-h-48 object-cover rounded mb-2">`;
        if (type === 'video' && value) {
            const id = extractYouTubeID(value);
            if (id) {
                el.innerHTML = `
                    <iframe class="w-full max-h-48 rounded mb-2"
                            src="https://www.youtube.com/embed/${id}"
                            frameborder="0" allowfullscreen></iframe>`;
            } else {
                el.innerHTML = `
                    <video class="w-full max-h-48 rounded mb-2" controls>
                        <source src="${value}">
                    </video>`;
            }
        }
...
// Función para enviar blog (Publicar o Borrador)
async function submitBlog(estado) {
    const nombre = document.getElementById('blogName').value;
    if (!nombre) return alert('El blog necesita un nombre');
    ...
    formData.append('estado', estado); // <-- clave para guardar borrador o publicar
...
// Eventos de botones
document.getElementById('publishBtn').addEventListener('click', () => submitBlog('publicado'));
document.getElementById('draftBtn').addEventListener('click', () => submitBlog('borrador'));
